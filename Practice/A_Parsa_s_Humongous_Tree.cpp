...
void dfs(int root, vector<int> *edges, int parent) {
    ...
    for(int i: edges[root]) {
        if(i != parent) {
            ll op1 = abs(vals[root].fr - vals[i].fr) + dp[i][0];
            ll op2 = abs(vals[root].fr - vals[i].sc) + dp[i][1];
            ans1 += max(op1, op2);

            op1 = abs(vals[root].sc - vals[i].fr) + dp[i][0];
            op2 = abs(vals[root].sc - vals[i].sc) + dp[i][1];
            ans2 += max(op1, op2);
        }
    }
