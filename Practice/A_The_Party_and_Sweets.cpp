...
int32_t main() {
    ...
    rep(test_case, 1, tc+1) {
        ...
        int mx = 0;
        rep(i, 0, n) mx = max(mx, b[i]);
        int mn = inf;
        rep(j, 0, m) mn = min(mn, g[j]);
        ...
        if(mn >= mx) {
            ...
            rep1(i, n-1, 0) {
                int cnt = 0;
                auto itr = lower_bound(all(g), b[i]);
                if(itr != g.end() && (*itr != b[i] && itr == g.begin() && end == m)) {
                    rep(j, itr-g.begin()+1, end) {
                        ans += g[j];
                        cnt++;
                    }
                    end = itr - g.begin()+1;
                } else {
                    rep(j, itr - g.begin(), end) {
                        ans += g[j];
                        cnt++;
                    }
                    end = itr - g.begin();
