...
int32_t main() {
    ...
    rep(test_case, 1, tc+1) {
        ...
        mem0(grid);
        int v = 1, h = 1;
        vector<pii> options = {{}, {2, 3}, {1, 3}, {1, 2}};
        rep(i, 0, n*n) {
            int given;
            cin>>given;

            if(i == 0) {
                grid[1][1] = options[given].fr;
            } else if(i == 1) {
                grid[2][1] = (options[given].fr == grid[1][1]) ? options[given].sc : options[given].fr;
                v++;
            } else if(h == 1) {
                pii temp = {grid[1][v-1], grid[2][v-1]};
                if(grid[h][v] != 0 || grid[h+1][v] != 0) {
