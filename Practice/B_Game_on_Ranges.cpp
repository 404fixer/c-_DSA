...
int32_t main() {
    ...
    rep(test_case, 1, tc+1) {
        ...
        rep(i, 0, n) {
            cin>>a[i].fr>>a[i].sc;
            pq.insert(a[i]);
            qp.insert({a[i].sc, a[i].fr});
        }

        vector<vi> res;
        rep(i, 0, n) {
            int ans;
            rep(j, a[i].fr, a[i].sc+1) {
                if(a[i].fr == a[i].sc) {
                    ans = a[i].fr;
                    break;
                }
                if(j == a[i].fr) {
                    auto itr = pq.find({a[i].fr + 1, a[i].sc});
                    if(itr != pq.end()) {
                        ans = j;
                        break;
                    }
                    ...
                    auto itr = pq.find({a[i].fr, j-1});
                    auto itr2 = pq.find({j+1, a[i].sc});
                    if(itr != pq.end() && itr2 != pq.end()) {
                        ans = j;
                        break;
                    }
                }
            }
            res.pb({a[i].fr, a[i].sc, ans});
        }   
