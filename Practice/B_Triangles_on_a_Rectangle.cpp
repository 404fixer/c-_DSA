...
int32_t main() {
    ...
    rep(test_case, 1, tc+1) {
        ...
        int k;
        cin>>k;
        vi x(k); rep(i, 0, k) cin>>x[i];
        cin>>k;
        vi xx(k); rep(i, 0, k) cin>>xx[i];
        cin>>k;
        vi y(k); rep(i, 0, k) cin>>y[i];
        cin>>k;
        vi yy(k); rep(i, 0, k) cin>>yy[i];

        ll ans = max((x[x.size()-1] - x[0])*h, (y[y.size()-1]-y[0])*w);
        ll ans2 = max((xx[xx.size()-1] - xx[0])*h, (yy[yy.size()-1]-yy[0])*w);
        ans = max(ans, ans2);
        cout<<ans<<endl;
    }
}