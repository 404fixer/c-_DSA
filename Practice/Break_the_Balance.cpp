...
int32_t main() {
    ...
    rep(test_case, 1, tc+1) {
        ...
        vi prefSum = {0};
        rep(i, 0, n) {
            if(s[i] == ')') prefSum.pb(prefSum.back()+i);
        }

        int closed = 0, open = 0;
        ll ans = inf;
        rep(i, 0, n) {
            ll req_closed = open - closed + 1;

            if(req_closed + closed > n/2) break;

            ll front_cost = prefSum[req_closed + closed] - prefSum[closed] - req_closed*(req_closed-1)/2;
            ll shift_cost = req_closed*i;
            ll total_cost = front_cost - shift_cost;
