    ...
    rep(test_case, 1, tc+1) {
        ...
        rep1(i, n-1, 0) {
            ...
            if(s[i] == 'a' && cnt > 0) {
                ...
                if(curr * (cnt * k + 1) >= x) {
                    // debug(i);
                    rep(j, 0, i+1) if(s[j] == 'a') res.pb(s[j]);
                    val[i+1] = curr;
                    cnt1[i+1] = cnt;
                    rep(j, i+1, n) {
                        if(s[j] == '*' && s[j-1] == 'a' && val[j]*(cnt1[j]*k + 1) >= x) {
                            ll curr_taken = x/val[j];
                            if(x%val[j] == 0) curr_taken--;
                            while(curr_taken--) res.pb('b');
                            x = x%val[j];
                            if(x == 0) {
                                rep(kk, j+1, n) {
                                    if(s[kk] == 'a') res.pb('a');
                                    else if(s[kk] == '*' && s[kk-1] == 'a') {
                                        ll temp = cnt1[kk]*k;
                                        while(temp--) res.pb('b');
                                    }
                                }
                ...
                if(i == 0) {
                    ...
                    if(curr * (cnt * k + 1) >= x) {
                        ...
                        // cnt1[i+1] = cnt;
                        rep(j, i, n) {
                            if((j == 0) || (s[j] == '*' && s[j-1] == 'a' && val[j]*(cnt1[j]*k + 1) >= x)) {
                                // debugg(j, x);
                                // debugg(val[j], cnt1[j]);
                                ll curr_taken = x/val[j];
                                if(x%val[j] == 0) curr_taken--;
