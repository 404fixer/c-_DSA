...
int32_t main() {
    ...
    rep(test_case, 1, tc+1) {
        ...
        bool ans = 1;
        int l = h[0], high = h[0] + k;
        rep(i, 1, n) {
            int te_low = h[i], te_high = h[i] + 2LL*k - 1LL;
            if(i == n-1) te_high = h[i] + k;
            if(h[i] < high && h[i] > l - k - (k-1)) {
                l = max(h[i], l - (k - 1));
                high = min(te_high, high + k - 1);
            } else {
                ans = 0;
                break;
            }

            if(i == n-1 && (high - l < k || l < h[i] || l > h[i] || high > h[i] + k || high < h[i] + k)) {
                ans = 0;
                break;
            }
        }
