...
int32_t main() {
    ...
    rep(test_case, 1, tc+1) {
        int n;
        cin>>n;
        vector<pair<pii, int>> ab(n);
        rep(i, 0, n) cin>>ab[i].fr.fr;
        rep(i, 0, n) cin>>ab[i].fr.sc, ab[i].sc = i;

        sort(all(ab));

        vi prefix_mx(n);
        prefix_mx[0] = ab[0].fr.sc;
        rep(i, 1, n) {
            prefix_mx[i] = max(prefix_mx[i-1], ab[i].fr.sc);
        }
