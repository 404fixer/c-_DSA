...
ll helper(vi a, int n, int k, bool govi) {
    ll ans = 0;
...
int32_t main() {
    ...
    rep(test_case, 1, tc+1) {
        ...
        rep(i, 0, n) {
            if(a[i] > 0) te.pb(a[i]);
            else if(a[i] < 0) te2.pb(-1LL*a[i]);
        }

        ll ans = 0;
        ll ans1 = 0;
        ans1 = helper(te, te.size(), k, 0);
        ans1 += helper(te2, te2.size(), k, 1);

        ans = helper(te2, te2.size(), k, 0);
        ans += helper(te, te.size(), k, 1);
