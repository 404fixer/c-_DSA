...
int32_t main() {
    ...
    rep(test_case, 1, tc+1) {
        ...
        while(start <= end) {
            ...
            if (mid%2 == 0) {
                if(mid-1 >= start) mid--;
                else if(mid+1 <= end) mid++;
                else break;
            }
            ld avg_te = (avg*((ld)mid-1) + (ld)h)/(ld)mid;
            // debugg(mid, avg_te);
                // debugg(ans, diff);
            if((ld)abs(avg_te - (ld)t) < diff) {
                ans = mid;
                diff = abs(avg_te - (ld)t);
                // debugg(ans, diff);
            } else if(abs(avg_te - (ld)t) == diff) {
                ans = min(ans, mid);
            }
        ...
        if(abs(avg - t) <= diff) ans = min(ans, 2LL);
