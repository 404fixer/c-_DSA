...
int32_t main() {
    ...
    rep(test_case, 1, tc+1) {
        ...
        ll dp[k][2];
        rep(i, 0, k) {
            if(i == 0) dp[i][1] = expo(2, k-i-1, modc);
            else dp[i][1] = mod_mul(dp[i-1][0], n/2, modc);
            
            if(i == 0) dp[i][0] = n/2;
            else dp[i][0] = mod_mul(dp[i-1][0], n/2, modc);
        }

        ll ans = mod_add(dp[k-1][1], dp[k-1][0], modc);
        cout<<ans<<endl;
    }
}