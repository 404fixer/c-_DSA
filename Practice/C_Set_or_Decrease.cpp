...
ll solve(vi &a, ll k, ll n, ll total) {
    ll ans = max(total - k, 0LL);
    ll sum = 0, cnt = 0;
    rep1(i, n-1, 1) {
        ll remaining = (total - sum) - k;
        sum += a[i] - a[0];
        if(remaining <= 0) ans = min(ans, cnt);
        else {
            remaining = (total - sum) - k;
            if(remaining <= 0) ans = min(ans, cnt+1);
            else {
                ll temp = n-i+1;
                ll te = (ll)ceil((ld)remaining/temp);
                ans = min(ans, te + cnt+1);
            }
        }
        cnt++;
    }
