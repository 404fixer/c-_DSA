...
int solve(vi &ones, vi &zeroes, ll i, ll j) {
    if(i == ones.size()) {
        return 0;
    }
    if(j == zeroes.size()) {
    ...
    int op1 = abs(ones[i]-zeroes[j]) + solve(ones, zeroes, i+1, j+1);
    int op2 = solve(ones, zeroes, i, j+1);
...
int32_t main() {
    ...
    rep(test_case, 1, tc+1) {
        ...
        mem1(dp);
        int ans = solve(ones, zeroes, 0LL, 0LL);
        cout<<ans<<endl;
    }
}