...
auto possible(int min_val, vector<vi> &shops, int m, int n) {
    bool can_n_1 = 0, can_n = 0;
    set<int> shops_taken;
    rep(i, 0, n) {
        can_n = 0;
        rep(j, 0, m) {
            if(shops[j][i] >= min_val) {
                can_n = 1;
                if(shops_taken.count(j)) can_n_1 = 1;
                shops_taken.insert(j);
            }
        }
        if(!can_n) break;
    }

    if(can_n_1 && can_n) return 1;
    return 0;
}
...
int32_t main() {
    ...
    rep(test_case, 1, tc+1) {
        int m, n;
        cin>>m>>n;
        vector<vi> shops(m, vi(n));
        rep(i, 0, m) rep(j, 0, n) cin>>shops[i][j];

        int low = 0, high = inf;
        ll ans = 0;
        while(low <= high) {
            int mid = low + (high - low)/2;
            if(possible(mid, shops, m, n)) {
                ans = mid;
                low = mid + 1;
            } else {
                high = mid - 1;
