...
bool check(int x, int y, int n, int m) {
    if(x < n && y < m && s[x][y] == '.') return 1;

    return 0;
}

int dfs(int x, int y, int ans, int n, int m) {
    int ans1 = ans, ans2 = ans;

    if(dp[x][y] != -1) return dp[x][y];

    if(check(x+1, y, n, m)) ans1 = dfs(x+1, y, ans + 1, n, m);
    if(check(x, y+1, n, m)) ans2 = dfs(x, y+1, ans + 1, n, m);
