...
int32_t main() {
    ...
    fact[0] = 1;
    rep(i, 1, N) {
        fact[i] = mod_mul(fact[i-1], i, modc);
    }
    ...
    rep(test_case, 1, tc+1) {
        ...
        rep(i, 0, n) {
            ll high_end = a[i] + k;
            auto itr = upper_bound(all(a), high_end);
            ll num_of_options = (itr - a.begin()) - i - 1LL;
            if(num_of_options >= m-1LL) ans = mod_add(ans, ncr(num_of_options, m-1LL, modc), modc);
        }
